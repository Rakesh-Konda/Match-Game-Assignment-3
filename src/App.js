...
class App extends Component {
  ...
  changeImages = () => {
    const {images, tabId} = this.state
    const FilteredImages = images.filter(each => each.category === tabId)
  ...
  ListImage = id => {
    ...
    const hlo = initialImg.id === id
    const {score} = this.state
    if (hlo) {
      const {images} = this.state
      const images1 = images
      const RandomImage = images1.sort(() => Math.random() - 0.5)
      console.log(RandomImage)
      this.setState({initialImg: RandomImage[5]})
      this.setState({score: score + 1, click: false})
    } else {
      this.setState(prevState => ({
        scorePage: !prevState.scorePage,
      }))
      clearInterval(this.timerId)
    }
  }
  ...
  render() {
    ...
    const ActualImages = this.changeImages()
        ...
        {scorePage ? (
          <div className="bg">
            <nav className="nav">
              <ul className="ull nav">
                ...
                <li className="timerLogo">
                  <p className="ff">
                    Score: <span className="ff">{click ? '0' : score}</span>
                  </p>
                  <div className="tim">
                    <img
                      className="timer"
            ...
            <div className="imMid">
              <img className="cenImg" src={initialImg.imageUrl} alt="match" />
            </div>

            <ul className="names">
              <li>
                <button type="button" className="bv" onClick={this.Fruits}>
                  <p className={fruitCol ? 'p' : 'pp'}>Fruits</p>
                </button>
              </li>
              <li>
                <button type="button" className="bv" onClick={this.Animals}>
                  <p className={animalCol ? 'p' : 'pp'}>Animals</p>
                </button>
              </li>
              <li>
                <button type="button" className="bv" onClick={this.Places}>
                  <p className={placeCol ? 'p' : 'pp'}>Places</p>
                </button>
              </li>
            </ul>
            ...
            <div className="ulCen">
              <ul className="ul">
                {ActualImages.map(each => (
                  <li key={each.id}>
                    <button
                      type="button"
                      className="bv"
                      onClick={() => this.ListImage(each.id)}
                    >
                      <img
                        className="smallImg"
          ...
          <div className="ll">
            <div className="uu">
              ...
              <div className="cenn">
                <p className="jj">YOUR SCORE</p>
                <h1 className="jj">{score}</h1>
                <button className="bu" type="button" onClick={this.PlayAgain}>
                  <img
